...
from .xml_parser import PersonalityConfig, config_to_xml, parse_xml_config

class PersonalityManager:
    """Manages personality configurations using file-based storage."""
    ...
    def _load_default_personalities(self):
        """Load default personalities if none exist."""
        if not self.list_personalities():
    ...
    def _create_default_personalities(self):
        ...
        for name, config in default_personalities.items():
            self.save_personality(name, config)
        
        # Set Mia as the default active personality
        self.set_active_personality("Mia")
    ...
    def save_personality(self, name: str, config: Dict[str, Any]) -> bool:
        """Save a personality configuration."""
        try:
            file_path = self.storage_dir / f"{name}.json"
    ...
    def set_active_personality(self, name: str) -> bool:
        """Set the active personality."""
        if self.get_personality(name):
    ...
    def get_active_personality(self) -> Optional[Dict[str, Any]]:
        ...
        if self.active_personality:
            return self.get_personality(self.active_personality)
        return None
    ...
    def get_categories(self) -> List[str]:
        ...
        for name in self.list_personalities():
            ...
            if personality and 'category' in personality:
                categories.add(personality['category'])
    ...
    def get_tags(self) -> List[str]:
        ...
        for name in self.list_personalities():
            personality = self.get_personality(name)
            if personality and 'tags' in personality:
                tags.update(personality['tags'])
        return sorted(list(tags))
    
    def search_personalities(self, query: str = None, category: str = None, tags: List[str] = None) -> List[Dict[str, Any]]:
        """Search personalities by query, category, or tags."""
        results = []
        ...
        for name in self.list_personalities():
            ...
            if query:
                ...
                if not (query_lower in personality.get('name', '').lower() or 
                       query_lower in personality.get('description', '').lower()):
                    continue
            
            # Filter by category
            if category and personality.get('category') != category:
                continue
            
            # Filter by tags
            if tags:
                personality_tags = set(personality.get('tags', []))
                if not any(tag in personality_tags for tag in tags):
                    continue
